<cfcomponent
	...
	<cffunction name="getContact" returntype="contact" access="public">
		<cfscript>
			var local = structNew();
			var c = createObject("component", "alert.com.contact").init();
			if(len(trim(variables.instance.primaryEmail))){
				 local.pe = createObject("component", "alert.com.email").init(variables.instance.primaryEmail, 1);
				 c.addEmailAddress(local.pe);
			}
			if(len(trim(variables.instance.secondaryEmail))){
				 local.se = createObject("component", "alert.com.email").init(variables.instance.secondaryEmail, 2, 2);
				 c.addEmailAddress(local.se);
			}
			if(len(trim(variables.instance.homePhone))){
				 local.hp = createObject("component", "alert.com.phone").init(variables.instance.homePhone, 1, 1);
				 c.addPhone(local.hp);
			}
			if(len(trim(variables.instance.workPhone))){
				 local.wp = createObject("component", "alert.com.phone").init(variables.instance.workPhone, 2, 2);
				 c.addPhone(local.wp);
			}
			if(len(trim(variables.instance.mobilePhone))){
				 local.mp = createObject("component", "alert.com.phone").init(variables.instance.mobilePhone, 2, 3);
				 c.addPhone(local.mp);
			}

			if(len(trim(variables.instance.textNumber))){
				 local.s = createObject("component", "alert.com.sms").init(variables.instance.textNumber, 1, 1);
